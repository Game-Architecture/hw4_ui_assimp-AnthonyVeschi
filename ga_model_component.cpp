...
void ga_mesh::get_vertices(aiMesh* mesh)
{
	...
	for (int i = 0; i < mesh->mNumVertices; i++)
	{
		...
		if (mesh->HasTextureCoords(0))
		{
			...
			_texcoords.push_back({ mesh->mTextureCoords[0][i].x, mesh->mTextureCoords[0][i].y });
		}
		if (mesh->HasNormals())
		{
	...
	for (int f = 0; f < mesh->mNumFaces; f++)
	{
		...
		_index_array.push_back(mesh->mFaces[f].mIndices[0]);
		_index_array.push_back(mesh->mFaces[f].mIndices[1]);
		_index_array.push_back(mesh->mFaces[f].mIndices[2]);
	}
...
void ga_mesh::create_from_aiMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	//color = (scene->mMaterials[mesh->mMaterialIndex])->;
